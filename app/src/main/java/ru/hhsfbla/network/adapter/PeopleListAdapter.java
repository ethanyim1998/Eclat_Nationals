...
//					api.follow(searchResults.get(getPosition).getId());
        ...
        if (u.getLowPhotoUrl().length() > 0 && u.getState() == Constants.ACCOUNT_STATE_ENABLED) {
			...
			String urlPre = u.getLowPhotoUrl();
			int index = urlPre.indexOf("/eclat") + 6;

            imageLoader.get("http://eclat.hhsfbla.com/" + urlPre.substring(index), ImageLoader.getImageListener(viewHolder.profilePhoto, R.drawable.profile_default_photo, R.drawable.profile_default_photo));
